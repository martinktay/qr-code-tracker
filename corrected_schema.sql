...
CREATE TABLE company_settings (
    ...
    preferred_languages message_language[] DEFAULT ARRAY['en'],
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Allow sender, recipient, and admin to select
CREATE POLICY "Allow sender, recipient, admin to read messages" ON messages
  FOR SELECT USING (
    auth.uid() = senderid OR
    auth.uid() = recipientid OR
    (EXISTS (SELECT 1 FROM user_accounts WHERE user_id = auth.uid() AND role = 'admin'))
  );
...
-- Allow sender to update their own message (e.g. for status)
...
-- Insert default company settings (only once)
INSERT INTO company_settings (company_name, contact_email, support_phone, terms_of_use, privacy_policy, enable_whatsapp, preferred_languages)
VALUES (
    'The Smart Exporters',
